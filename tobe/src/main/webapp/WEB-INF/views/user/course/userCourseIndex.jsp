...
<script>
      ...
      function updateCity(localcd) {
          ...
          cityButtons.removeClass('on'); // 모든 버튼에서 'on' 클래스 제거

          // 해당 지역에 속하는 지점에 'on' 클래스 추가
          cities.forEach(function(city) {
              cityButtons.filter('[data-citycd="' + city.cityCode + '"]').addClass('on');
          });
      }

      // 함수 정의: 선택한 지역에 따라 속하는 지점 가져오기
      function getCitiesByLocal(localcd) {
          // 여기에 각 지역에 속하는 지점들의 정보를 정의해두세요.
          var cities = [];

          switch (localcd) {
              case '2': // 서울
                  cities = [
                      { cityCode: '1', cityName: '강남' },
                      { cityCode: '2', cityName: '종로' },
                      { cityCode: '3', cityName: '종로e4u'},
                      { cityCode: '4', cityName: '신촌'},
                      { cityCode: '5', cityName: '건대'}
                  ];
                  break;
              case '32': // 인천
                  cities = [
                      { cityCode: '11', cityName: '인천센터' },
                  ];
                  break;
              case '51': // 부산
                  cities = [
                      { cityCode: '7', cityName: '서면' },
                      { cityCode: '8', cityName: '부산대' },
                      { cityCode: '9', cityName: '대연' },
                      { cityCode: '10', cityName: '광복' },
                  ];
                  break;
              case '53': // 대구
                  cities = [
                      { cityCode: '6', cityName: '동성로' },
                  ];
                  break;
          }
      ...
      $(document).ready(function() {
         $('#td_level button').on('click', function(){
            $(this).addClass('on').siblings().removeClass('on');
            var toeicLevelcd = $(this).data('toeicLevelcd');
            console.log('레벨 코드 : ' , toeicLevelcd);
         });
      });
      $(document).ready(function () {
          // 요일 버튼 클릭 시 상태 토글
